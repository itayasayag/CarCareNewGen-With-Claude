...
namespace AdHocObjects.Controllers
{
    ...
    public class UploadController : ControllerBase
    {
        ...
        {
            string path = Path.Combine(Directory.GetCurrentDirectory(), "uploadedFiles", imageName);
            if (System.IO.File.Exists(path))
            {
                var stream = new FileStream(path, FileMode.Open);
                return File(stream, "image/*"); // Adjust the MIME type according to your image type
            }
            else
            {
                return NotFound();
